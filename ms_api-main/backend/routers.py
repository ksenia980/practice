...
@router.get("/types/contains-izdeliye", tags=["🏷️ Типы экспонатов"])
def get_types_containing_izdeliye(db: Session = Depends(get_db)):
    """Вывести типы экспонатов, содержащие слово 'изделие'"""
    types = db.query(models.Type).all()
    ...
    if not result:
        return create_response_with_sql({"message": "Не найдено", "found": 0})
...
@router.get("/analytics/types-with-count", tags=["📊 Аналитика"])
def get_types_with_item_count(db: Session = Depends(get_db)):
    """Найти типы экспонатов и количество предметов каждого типа"""
    from sqlalchemy import func
    
    # JOIN типов с экспонатами и подсчет
    results = db.query(
        models.Type.id,
        models.Type.name,
        func.count(models.Wing.id).label('item_count')
    ).outerjoin(
        models.Wing, models.Type.id == models.Wing.type_id
    ).group_by(
        models.Type.id, models.Type.name
    ).order_by(
        func.count(models.Wing.id).desc()
    ).all()
    
    result = []
    for type_id, type_name, count in results:
        result.append({
            "type_id": type_id,
            "type_name": type_name,
            "item_count": count
...
@router.get("/moves/top-expensive", tags=["🚚 Перемещения"])
def get_top_expensive_moves(
    min_price: float = 30000,
    limit: int = 10,
    db: Session = Depends(get_db)
):
    """Найти N самых дорогих (дороже min_price) экспонатов"""
    moves = db.query(models.Move).filter(
        models.Move.price > min_price
    ).order_by(
        models.Move.price.desc()
    ).limit(limit).all()
    
    if not moves:
        raise HTTPException(status_code=404, detail=f"Перемещения дороже {min_price} не найдены")
    
    result = []
    for move in moves:
        # Получаем информацию об экспонате и месте
        wing = move.wing
        place = move.place
        ...
        result.append({
            "move_id": move.id,
            "wing_id": move.wing_id,
            "wing_name": wing.name if wing else "Неизвестно",
            "price": move.price,
            "place_id": move.place_id,
            "place_location": place.location if place else "Неизвестно",
            "date": move.dt.isoformat() if move.dt else None
        })
